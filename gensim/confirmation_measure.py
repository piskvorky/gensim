...
def log_conditional_probability(segmented_topics, per_topic_probability):
    ...
    per_topic_probability : Output from the probability_estimation module. Is a dictionary of the probability estimated
                            on all topics.
    ...
    for s_i in segmented_topics:
        for w_prime, w_star in s_i: # Have to generalise this to all kinds of sets. Currently suited for S_One_One style segmentation.
            m_lc_i = np.log(((per_topic_probability[w_prime] * per_topic_probability[w_star]) + EPSILON) / per_topic_probability[w_star])
            m_lc.append(m_lc_i)
