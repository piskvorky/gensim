...
class DmlSource(ArticleSource):
    ...
    def findArticles(self):
        ...
        logger.info(
            "%i directories processed, found %i articles", dirTotal, artAccepted
        )
    ...
    def tokenize(self, content):
        return [
            token.encode("utf8")
            for token in utils.tokenize(content, errors="ignore")
            if not token.isdigit()
        ]

    def normalizeWord(self, word):
        wordU = unicode(word, "utf8")
        return wordU.lower().encode(
            "utf8"
...
class ArxmlivSource(ArticleSource):
    ...
    class ArxmlivContentHandler(xml.sax.handler.ContentHandler):
        ...
        def characters(self, text):
            ...
            if self.path[-1] == "p":
                tokens = [
                    token.encode("utf8")
                    for token in utils.tokenize(text, errors="ignore")
                    if not token.isdigit()
                ]
    ...
    def getMeta(self, uri):
        ...
        return {
            "language": "eng"
        }  # TODO maybe parse out some meta; but currently not needed for anything...
