...
# prune based on similarity score (all below MIN_SCORE are ignored)
MIN_SCORE = 0.0
# prune based on rank (at most MAX_SIMILAR are stored). set to 0 to store
# all of them (no limit).
MAX_SIMILAR = 10

# if there are no similar articles (after the pruning), do we still want
# to generate similar.xml?
SAVE_EMPTY = True
...
# template for the whole similar.xml file (will be filled with multiple
# ARTICLE instances)
SIMILAR = """\
    ...
    for docNo, topSims in enumerate(index):  # for each document
        ...
        if SAVE_EMPTY or articles:
            ...
            if not DRY_RUN:  # only open output files for writing if DRY_RUN is false
                logging.info(
                    "generating %s (%i similars)" %
                    (outfile, len(articles)))
                outfile = open(outfile, 'w')
                # add xml headers and print to file
                outfile.write(SIMILAR % output)
                outfile.close()
            else:
                logging.info(
                    "would be generating %s (%i similars):%s\n" %
...
if __name__ == '__main__':
    ...
    assert len(input) == len(
        corpus), "corpus size mismatch (%i vs %i): run ./gensim_genmodel.py again" % (len(input), len(corpus))
    ...
    # do not normalize query vectors during similarity queries (the index is
    # already built normalized, so it would be a no-op)
    index.normalize = False
    # for each document, print MAX_SIMILAR nearest documents to a xml file, in
    # dml-cz specific format
