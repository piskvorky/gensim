...
def word2vec2tensor(word2vec_model_path, tensor_filename, binary=False):
    ...
    with utils.open(outfiletsv, "wb") as file_vector, utils.open(
        outfiletsvmeta, "wb"
    ) as file_metadata:
        for word in model.index_to_key:
            ...
            file_vector.write(
                gensim.utils.to_utf8(vector_row) + gensim.utils.to_utf8("\n")
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        formatter_class=argparse.RawDescriptionHelpFormatter, description=__doc__[:-138]
    )
    parser.add_argument(
        "-i", "--input", required=True, help="Path to input file in word2vec format"
    )
    parser.add_argument(
        "-o", "--output", required=True, help="Prefix path for output files"
    )
    parser.add_argument(
        "-b",
        "--binary",
