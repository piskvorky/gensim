...
if __name__ == "__main__":
    ...
    logger.info(
        'Converted model with %i vectors and %i dimensions',
        num_lines,
        num_dims)
    # test that the converted model loads successfully
    model = gensim.models.Word2Vec.load_word2vec_format(
        args.output, binary=False)
    logger.info('Model %s successfully loaded', model)
    try:
        logger.info('testing the model....')
        if sys.version_info < (3,):
            with smart_open(args.output, 'rb') as f:
                seed_word1, seed_word2 = random.sample(
                    [line.split()[0] for line in f], 2)
        else:
            with smart_open(args.output, 'r') as f:
                seed_word1, seed_word2 = random.sample(
                    [line.split()[0] for line in f], 2)
        logger.info(
            'top-10 most similar words to "%s" are: %s',
            seed_word1,
            model.most_similar(
                positive=[seed_word1],
                topn=10))
        logger.info(
            'similarity score between %s and %s: %s',
            seed_word1,
            seed_word2,
            model.similarity(
                seed_word1,
        ...
        logger.error(
            'error encountered. checking for model file creation now....')
