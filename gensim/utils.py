...
class SaveLoad(object):
    ...
    def _load_specials(self, fname, mmap, compress, subname):
        ...
        for attrib in getattr(self, '__nps', []):
...
class InputQueue(multiprocessing.Process):
    def __init__(self, q, corpus, chunksize, maxsize, as_np):
...
if os.name == 'nt':
    ...
    def chunkize(corpus, chunksize, maxsize=0, as_np=False):
        for chunk in chunkize_serial(corpus, chunksize, as_np=as_np):
            yield chunk
else:
    def chunkize(corpus, chunksize, maxsize=0, as_np=False):
        """
        ...
        if maxsize > 0:
            ...
            worker = InputQueue(q, corpus, chunksize, maxsize=maxsize, as_np=as_np)
            worker.daemon = True
            worker.start()
            while True:
