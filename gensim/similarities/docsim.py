...
The main class is `Similarity`, which builds an index for a given set of
documents. Once the index is built, you can perform efficient queries like
"Tell me how similar is this query document to each document in the index?".
The result is a vector of numbers as large as the size of the initial set of
documents, that is, one float for each index document. Alternatively, you can
also request only the top-N most similar index documents to the query.
...
("shards"), which are disk-based. If your entire index fits in memory
(~hundreds of thousands documents for 1GB of RAM), you can also use the
`MatrixSimilarity` or `SparseMatrixSimilarity` classes directly. These are more
simple but do not scale as well (they keep the entire index in RAM, no
sharding).

Once the index has been initialized, you can query for document similarity
simply by:

>>> index = Similarity('/tmp/tst', corpus, num_features=12) # build the index
>>> similarities = index[query] # get similarities between the query and all
index documents

If you have more query documents, you can submit them all at once, in a batch:

>>> for similarities in index[batch_of_documents]: # the batch is simply an
iterable of documents (=gensim corpus)
>>>     ...
...
<http://groups.google.com/group/gensim/msg/4f6f171a869e4fca?>`_).

There is also a special syntax for when you need similarity of documents in the
index to the index itself (i.e. queries=indexed documents themselves). This
special syntax uses the faster, batch queries internally and **is ideal for
all-vs-all pairwise similarities**:

>>> for similarities in index: # yield similarities of the 1st indexed
document, then 2nd...
>>>     ...
...
try:
    ...
    # by default, don't parallelize queries. uncomment the following line if
    # you want that.
    # use parallel processes = #CPus
#    PARALLEL_SHARDS = multiprocessing.cpu_count()
except ImportError:
    pass
...
class Shard(utils.SaveLoad):
    ...
    def __str__(self):
        return ("%s Shard(%i documents in %s)" % (self.cls.__name__, len(self),
                                                  self.fullname()))
    ...
    def __getitem__(self, query):
        ...
        try:
            ...
            raise ValueError("num_best and normalize have to be set before"
                             " querying a proxy Shard object")
        return index[query]


def query_shard(args):
    # simulate starmap (not part of multiprocessing in older Pythons)
    query, shard = args
    logger.debug("querying shard %s num_best=%s in process %s", shard,
                 shard.num_best, os.getpid())
    result = shard[query]
    logger.debug("finished querying shard %s in process %s", shard,
                 os.getpid())
    return result
...
class Similarity(interfaces.SimilarityABC):
    """
    Compute cosine similarity of a dynamic query against a static corpus of
    documents ("the index").

    Scalability is achieved by sharding the index into smaller pieces, each of
    which fits into core memory (see the `(Sparse)MatrixSimilarity` classes in
    this module). The shards themselves are simply stored as files to disk and
    mmap'ed back as needed.

    """
    def __init__(self, output_prefix, corpus, num_features, num_best=None,
                 chunksize=256, shardsize=32768):
        """
        Construct the index from `corpus`. The index can be later extended by
        calling the `add_documents` method. **Note**: documents are split
        (internally, transparently) into shards of `shardsize` documents each,
        converted to a matrix, for faster BLAS calls. Each shard is stored to
        disk under `output_prefix.shard_number` (=you need write access to that
        location). If you don't specify an output prefix, a random filename in
        temp will be used.

        `shardsize` should be chosen so that a `shardsize x chunksize` matrix
        of floats fits comfortably into main memory.

        `num_features` is the number of features in the `corpus` (e.g. size of
        the dictionary, or the number of latent topics for latent semantic
        models).

        If `num_best` is left unspecified, similarity queries will return a
        full vector with one float for every document in the index:
        ...
        If `num_best` is set, queries return only the `num_best` most similar
        documents, always leaving out documents for which the similarity is 0.
        If the input vector itself only has features with zero values (=the
        sparse representation is empty), the returned list will always be
        empty.
        ...
        if output_prefix is None:
            # undocumented feature: set output_prefix=None to create the server
            # in temp
    ...
    def add_documents(self, corpus):
        ...
        if self.shards and len(self.shards[-1]) < min_ratio * self.shardsize:
            # The last shard was incomplete (<; load it back and add the
            # documents there, don't start a new shard)
            self.reopen_shard()
        for doc in corpus:
    ...
    def close_shard(self):
        ...
        **NOTE**: the shard is closed even if it is not full yet (its size is
        smaller than `self.shardsize`). If documents are added later via
        `add_documents()`, this incomplete shard will be loaded again and
        completed.
        """
        ...
        if issparse:
            index = SparseMatrixSimilarity(self.fresh_docs,
    ...
    def reopen_shard(self):
        assert self.shards
        if self.fresh_docs:
            raise ValueError("cannot reopen a shard with fresh documents in"
                             " index")
    ...
    def query_shards(self, query):
        """
        Return the result of applying shard[query] for each shard in
        self.shards, as a sequence.
    ...
    def __getitem__(self, query):
        ...
        **or**
        ...
        If `query` is a corpus (iterable of documents), return a matrix of
        similarities of all query documents vs. all corpus document. This batch
        query is more efficient than computing the similarities one document
        after another.
        """
        self.close_shard()  # no-op if no documents added to index since last query

        # reset num_best and normalize parameters, in case they were changed
        # dynamically
        for shard in self.shards:
            shard.num_best = self.num_best
            shard.normalize = self.normalize

        # there are 4 distinct code paths, depending on whether input `query`
        # is a corpus (or numpy/scipy matrix) or a single document, and whether
        # the similarity result should be a full array or only num_best most
        # similar documents.
        pool, shard_results = self.query_shards(query)
        if self.num_best is None:
            # user asked for all documents => just stack the sub-results into a
            # single matrix (works for both corpus / single doc query)
            result = numpy.hstack(shard_results)
        else:
            # the following uses a lot of lazy evaluation and (optionally)
            # parallel processing, to improve query latency and minimize memory
            # footprint.
            ...
            if not is_corpus:
                ...
                # the trickiest combination: returning num_best results when
                # query was a corpus
                results = []
    ...
    def vector_by_id(self, docpos):
        """
        Return indexed vector corresponding to the document at position
        `docpos`.
        """
        self.close_shard()  # no-op if no documents added to index since last query
        pos = 0
    ...
    def iter_chunks(self, chunksize=None):
        """
        Iteratively yield the index as chunks of documents, each of
        size <= chunksize.

        The chunk is returned in its raw form (matrix or sparse matrix slice).
        The size of the chunk may be smaller than requested; it is up to the
        caller to check the result for real length, using `chunk.shape[0]`.
        """
        self.close_shard()
        ...
        for shard in self.shards:
            ...
            for chunk_start in xrange(0, query.shape[0], chunksize):
                # scipy.sparse doesn't allow slicing beyond real size of the
                # matrix (unlike numpy). so, clip the end of the chunk
                # explicitly to make scipy.sparse happy
    ...
    def check_moved(self):
        """
        Update shard locations, in case the server directory has moved on
        filesystem.
        """
    ...
    def save(self, fname=None, *args, **kwargs):
        """
        Save the object via pickling (also see load) under filename specified
        in the constructor.

        Calls `close_shard` internally to spill any unfinished shards to disk
        first.
    ...
    def destroy(self):
        """
        Delete all files under self.output_prefix. Object is not usable after
        calling this method anymore. Use with care!
...
class MatrixSimilarity(interfaces.SimilarityABC):
    """
    Compute similarity against a corpus of documents by storing the index
    matrix in memory. The similarity measure used is cosine between two
    vectors.

    Use this if your input corpus contains dense vectors (such as documents in
    LSI space) and fits into RAM.

    The matrix is internally stored as a *dense* numpy array. Unless the entire
    matrix fits into main memory, use `Similarity` instead.

    See also `Similarity` and `SparseMatrixSimilarity` in this module.

    """
    def __init__(self, corpus, num_best=None, dtype=numpy.float32,
                 num_features=None, chunksize=256, corpus_len=None):
        """
        `num_features` is the number of features in the corpus (will be
        determined automatically by scanning the corpus if not specified). See
        `Similarity` class for description of the other parameters.

        """
        if num_features is None:
            logger.warning("scanning corpus to determine the number of"
                           " features (consider setting `num_features`"
                           " explicitly)")
        ...
        if corpus is not None:
            ...
            self.index = numpy.empty(shape=(corpus_len, num_features),
                                     dtype=dtype)
            # iterate over corpus, populating the numpy index matrix with
            # (normalized) document vectors
            for docno, vector in enumerate(corpus):
                if docno % 1000 == 0:
                    logger.debug("PROGRESS: at document #%i/%i", docno,
                                 corpus_len)
                # individual documents in fact may be in numpy.scipy.sparse
                # format as well. it's not documented because other it's not
                # fully supported throughout. the user better know what he's
                # doing (no normalization, must explicitly supply
                # num_features etc).
                if isinstance(vector, numpy.ndarray):
                    ...
                    vector = matutils.unitvec(matutils.sparse2full(vector,
                                                                   num_features
                                                                   ))
    ...
    def get_similarities(self, query):
        """
        Return similarity of sparse vector `query` to all documents in the
        corpus, as a numpy array.

        If `query` is a collection of documents, return a 2D array of
        similarities of each document in `query` to all documents in the corpus
        (=batch query, faster than processing each document in turn).
    ...
    def __str__(self):
        return "%s<%i docs, %i features>" % (self.__class__.__name__,
                                             len(self),
                                             self.index.shape[1])
...
class SparseMatrixSimilarity(interfaces.SimilarityABC):
    """
    Compute similarity against a corpus of documents by storing the sparse
    index matrix in memory. The similarity measure used is cosine between two
    vectors.

    Use this if your input corpus contains sparse vectors (such as documents in
    bag-of-words format) and fits into RAM.

    The matrix is internally stored as a `scipy.sparse.csr` matrix. Unless the
    entire matrix fits into main memory, use `Similarity` instead.
    ...
    def __init__(self, corpus, num_features=None, num_terms=None,
        ...
        if corpus is not None:
            ...
            try:
                ...
                # no MmCorpus, use the slower version (or maybe user supplied
                # the num_* params in constructor)
                pass
            ...
            if num_terms is None:
                raise ValueError("refusing to guess the number of sparse"
                                 " features: specify num_features explicitly")
            ...
            self.index = matutils.corpus2csc(
                corpus, num_terms=num_terms, num_docs=num_docs,
                num_nnz=num_nnz, dtype=dtype, printprogress=10000).T

            # convert to Compressed Sparse Row for efficient row slicing
            # and multiplications
    ...
    def get_similarities(self, query):
        ...
        **Do not use this function directly; use the self[query] syntax instead.**
        ...
        is_corpus, query = utils.is_corpus(query)
        if is_corpus:
            query = matutils.corpus2csc(query, self.index.shape[1],
                                        dtype=self.index.dtype)
        else:
            if scipy.sparse.issparse(query):
        ...
        # compute cosine similarity against every other document in the
        # collection
