...
class AnnoyIndexer(object):
    ...
    def __init__(self, model=None, num_trees=None):
        ...
        if model and num_trees:
            if isinstance(self.model, Doc2Vec):
                ...
                raise ValueError(
                    "Only a Word2Vec or Doc2Vec instance can be used")
    ...
    def load(self, fname):
        ...
        if not (os.path.exists(fname) and os.path.exists(fname_dict)):
            raise IOError(
                "Can't find index files '%s' and '%s' - Unable to restore AnnoyIndexer state." %
                (fname, fname_dict))
        else:
    ...
    def most_similar(self, vector, num_neighbors):
        ...
        return [(self.labels[ids[i]], 1 - distances[i] / 2)
