...
Example:
model=dtm.DTMmodel(corpus,data000,num_topics=20,id2word=dictionary.id2token)
>>> model = gensim.models.DTMmodel(corpus=my_corpus, num_topics=20, id2word=dictionary,dtm_path='dtm-win64.exe')
...
class DTMmodel(utils.SaveLoad):
    ...
    def __init__(
        ...
        if lencorpus == 0:
            raise ValueError(
                "cannot compute DTM over an empty corpus")
        if lencorpus != sum(time_slices):
            raise ValueError(
                "mismatched timeslices %{slices} for corpus of len {clen}".format(slices=sum(time_slices), clen=lencorpus))
    ...
    def convert_input(self, corpus, time_slices):
        ...
        corpora.BleiCorpus.save_corpus(
            self.fcorpustxt(), corpus)
    ...
    def train(self, corpus, time_slices, mode='fit', model='fixed'):
        ...
        arguments = "--ntopics={p0} --model={mofrl}  --mode={p1} --initialize_lda={p2} --corpus_prefix={p3} --outname={p4} --alpha={p5}".format(p0=self.num_topics, mofrl=model, p1=mode, p2=self.initialize_lda, p3=self.fcorpus(), p4=self.foutname(), p5=self.alpha)

        params = "--lda_max_em_iter={p0} --lda_sequence_min_iter={p1}  --lda_sequence_max_iter={p2} --top_chain_var={p3} --rng_seed={p4} ".format(p0=self.lda_max_em_iter,p1=self.lda_sequence_min_iter, p2=self.lda_sequence_max_iter, p3=self.top_chain_var, p4=self.rng_seed)
        ...
        try:
            p = Popen(
                [self.dtm_path] + arguments.split(), stdout=PIPE, stderr=PIPE)
            p.communicate()
        ...
        if model is 'fixed':
            for t in range(len(self.time_slices)):
                stamp = "%03d" % t
                self.influnces_time.append(
                    np.loadtxt(self.fout_influence().format(i=stamp)))
