...
class DRMM:
    def __init__(self, text_maxlen, vocab_size, embedding_matrix, hist_size=60, dropout_rate=0.,
        ...
        self.initializer_gate = keras.initializers.RandomUniform(
            minval=-0.01, maxval=0.01, seed=11)
    ...
    def build(self):
        ...
        q_w = Dense(1, kernel_initializer=self.initializer_gate,
                    use_bias=False)(q_embed)

        q_w = Lambda(lambda x: softmax(x, axis=1),
                     output_shape=(self.text_maxlen, ))(q_w)

        z = doc
        z = Dropout(rate=self.dropout_rate)(z)

        for i in range(self.num_layers - 1):
            z = Dense(self.hidden_sizes[i],
                      kernel_initializer=self.initializer_fc)(z)
            z = Activation('tanh')(z)

        z = Dense(self.hidden_sizes[self.num_layers - 1],
                  kernel_initializer=self.initializer_fc)(z)
        z = Permute((2, 1))(z)
