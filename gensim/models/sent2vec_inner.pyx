...
cdef (float, int) negative_sampling(const int target, const float lr, float *grad, float *wo, float *hidden,
    ...
    for i from 0 <= i <= neg:
        if i == 0:
            ...
            temp, new_negpos = get_negative(target, negatives, negpos, negatives_len)
            loss += binary_logistic(temp, label_false, lr, vector_size, wo, grad, hidden)
    return loss, new_negpos
...
cdef (int, int) get_negative(const int target, int *negatives,
                              int negpos, const int negatives_len):

    cdef int negative
    while True:
        negative = negatives[<int>negpos]
        negpos = (negpos + <int>ONE) % negatives_len
        if target != negative:
            break
    return negative, negpos
...
def update(model, context_, target_, lr_):
    ...
    loss_ , negpos = negative_sampling(target, lr, grad, wo, hidden, vector_size, neg, negatives, negpos, negatives_len)
    model.nexamples += 1
