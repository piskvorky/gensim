...
"""
Deep learning via doc2vec's "skip-gram and CBOW models", using either
hierarchical softmax or negative sampling [1]_ [2]_.

The training algorithms were originally ported from the C package https://code.google.com/p/doc2vec/
and extended with additional functionality.

**Install Cython with `pip install cython` to use optimized doc2vec training** (70x speedup [3]_).

Initialize a model with e.g.::

>>> model = Doc2Vec(sentences, size=100, window=5, min_count=5, workers=4)
...
.. [3] Optimizing doc2vec in gensim, http://radimrehurek.com/2013/09/doc2vec-in-python-part-two-optimizing/
"""
...
try:
    from gensim_addons.models.doc2vec_inner import train_sentence_sg, train_sentence_cbow, FAST_VERSION
except ImportError:
    try:
        ...
        from doc2vec_inner import train_sentence_sg, train_sentence_cbow, FAST_VERSION
    except:
        # failed... fall back to plain numpy (20-80x slower training than the above)
        FAST_VERSION = -1

        def train_sentence_sg(model, sentence, lbls, alpha, work=None):
            """
            ...
            for pos, word in enumerate(sentence):
                ...
                # TODO: do we want to add labels as elements to be predicted here? Might be fun to try...
                for pos2, word2 in enumerate(sentence[start : pos + model.window + 1 - reduced_window], start):
                    ...
                    if word2 and not (pos2 == pos):
                        ...
                        if model.hs:
                            # work on the entire tree at once, to push as much work into numpy's C routines as
                        ...
                        if model.negative:
                            # use this word (label = 1) + `negative` other random words not from this
                            # sentence (label = 0)
        ...
        def train_sentence_cbow(model, sentence, lbls, alpha, work=None, neu1=None):
            """
...
class DocSet(object):
    """
    Class for storing collections of documents. Enables training of vectors for documents, paragraphs, sentences, ...

    separators = set of elements separating various text strings you want to split on, ordered from largest text size
    to smallest.
    e.g. separators=["\n\n", "."] will split on paragraphs ("\n\n") and then on sentences (".")
    probabilities = odds of training on each of the text types produced by 'separators'.
    uids = unique pretokens for text elements.
    """
    def __init__(self,
                 doc_folder,
                 pattern,
                 ...
                 #probabilities=[],
                 uids=[]):
        assert len(separators) == len(uids)
        self.doc_folder = doc_folder
        self.pattern = pattern
        #self.doc_probability = doc_probability
        self.id_lookup_file = id_lookup_file
    ...
    def __split_iter(self, labeled_text, level=0):
        if len(self.separators) > level:
            texts = [x.strip() for x in labeled_text.text.split(self.separators[level]) if x.strip() is not ""]
            new_labels = map(lambda x: self.__get_next_id(level), texts)
            self.__write_ids(new_labels, texts)
    ...
    def __get_next_id(self, level):
        count = self.counters[level]
        self.counters[level] += 1
        return self.uids[level]+'\000'+str(count)
...
class Doc2Vec(utils.SaveLoad):
    ...
    def make_table(self, table_size=100000000, power=0.75):
        ...
        if not vocab_size:
            logger.warning("empty vocabulary in doc2vec, is this intended?")
            return
    ...
    def train(self, sentences, total_words=None, word_count=0, chunksize=100):
        ...
        def worker_train():
            ...
            while True:
                ...
                if self.sg:
                    job_words = sum(train_sentence_sg(self, sentence[0], sentence[1], alpha, work) for sentence in job)
                else:
                    job_words = sum(train_sentence_cbow(self, sentence[0], sentence[1], alpha, work, neu1) for sentence in job)
                with lock:
    ...
    def save_doc2vec_format(self, fname, fvocab=None, binary=False):
        ...
        C doc2vec-tool, for compatibility.
    ...
    @classmethod
    def load_doc2vec_format(cls, fname, fvocab=None, binary=False, norm_only=True):
        """
        Load the input-hidden weight matrix from the original C doc2vec-tool format.
    ...
    def most_similar(self, positive=[], negative=[], topn=10):
        ...
        `distance` scripts in the original doc2vec implementation.
    ...
    def accuracy(self, questions, restrict_vocab=30000):
        ...
        See https://code.google.com/p/doc2vec/source/browse/trunk/questions-words.txt for an example.
        ...
        This method corresponds to the `compute-accuracy` script of the original C doc2vec.
...
# Example: ./doc2vec.py ~/workspace/doc2vec/text8 ~/workspace/doc2vec/questions-words.txt ./text8
if __name__ == "__main__":
    ...
    model = Doc2Vec(Text8Corpus(infile), size=200, min_count=5, workers=1)
