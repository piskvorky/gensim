...
try:
    ...
    def train_sentence_dbow(model, word_vocabs, doclbl_indices, alpha, work=None,
        ...
        for doclbl_index in doclbl_indices:
            for word in word_vocabs:
                if word is None:
                    continue  # OOV word in the input sentence => skip
                train_sg_pair(model, word, doclbl_index, alpha, learn_vectors=learn_doclbls,
    ...
    def train_sentence_dm(model, word_vocabs, doclbl_indices, alpha, work=None, neu1=None,
    ...
    def train_sentence_dm_concat(model, word_vocabs, doclbl_indices, alpha, work=None, neu1=None,
                                 learn_doclbls=True, learn_words=True, learn_hidden=True,
                                 word_vectors=None, word_locks=None, doclbl_vectors=None, doclbl_locks=None):
        """
...
class DocvecsInArray(object):
    """
    Default storage of docvecs during training, in a numpy array.

    Maintains dict mapping string doclbl -> int mapping if necessary.
    (If all LabeledSentences use only int doclbls, this overhead is
    ...
    def __init__(self, mapfile_path=None):
        self.doclbls = {}  # string -> Doclbl (if necessary)
        self.index2doclbl = []  # int index -> String (if necessary)
        self.max_index = -1
        self.mapfile_path = mapfile_path

    def note_doclbl(self, key, sentence_no, sentence_length):
        if isinstance(key, int):
            self.max_index = max(self.max_index, key)
        else:
            if key in self.doclbls:
                self.doclbls[key] = self.doclbls[key].repeat(sentence_length)
            else:
                self.doclbls[key] = Doclbl(sentence_no, sentence_length, 1)
                self.index2doclbl.append(key)

    def indexed_doclbls(self, doclbl_tokens):
        return ([i for i in [self._int_index(index,-1) for index in doclbl_tokens] if i > -1],
                self.doclbl_syn0, doclbl_tokens)
    ...
    def _int_index(self, index, missing=None):
        if isinstance(index, int):
            ...
            return self.doclbls[index].index if index in self.doclbls else missing

    def __getitem__(self, index):
        return self.doclbl_syn0[self._int_index(index)]
    ...
    def reset_weights(self, model):
        ...
        for i in xrange(length):
            ...
            seed = "%d %s" % (model.seed, self.index2doclbl[i] if len(self.index2doclbl)>0 else str(i))
            self.doclbl_syn0[i] = model.seeded_vector(seed)
...
class Doc2Vec(Word2Vec):
    ...
    def __init__(self, sentences=None, size=300, alpha=0.025, window=8, min_count=5,
                 ...
                 dbow_words=0, dm_mean=0, dm_concat=0, dm_lbl_count=1,
        ...
        if not self.docvecs:
            self.docvecs = DocvecsInArray(docvecs_mapfile)
    ...
    def _vocab_from(self, sentences):
        ...
        for sentence_no, sentence in enumerate(sentences):
            ...
            for label in sentence.labels:
                self.docvecs.note_doclbl(label, sentence_no, sentence_length)
    ...
    def _prepare_sentences(self, sentences):
        for sentence in sentences:
            ...
            yield (self._tokens_to_vocabs(sentence.words),
                   self.docvecs.indexed_doclbls(sentence.labels))
    ...
    def _get_job_words(self, alpha, work, job, neu1):
        if self.sg:
            tally = sum(train_sentence_dbow(self, sentence, doclbl_indices, alpha, work, train_words=self.dbow_words,
                                           doclbl_vectors=doclbl_vectors)
                       for sentence, (doclbl_indices, doclbl_vectors, ignored) in job)
        elif self.dm_concat:
            tally = sum(train_sentence_dm_concat(self, sentence, doclbl_indices, alpha, work, neu1,
                                                doclbl_vectors=doclbl_vectors)
                       for sentence, (doclbl_indices, doclbl_vectors, ignored) in job)
        else:
            tally = sum(train_sentence_dm(self, sentence, doclbl_indices, alpha, work, neu1,
                                         doclbl_vectors=doclbl_vectors)
                       for sentence, (doclbl_indices, doclbl_vectors, ignored) in job)
    ...
    def infer_vector(self, document, alpha=0.1, min_alpha=0.0001, steps=5):
        ...
        for i in range(steps):
            if self.sg:
                train_sentence_dbow(self, word_vocabs, doclbl_indices, alpha, work,
                                    learn_words=False, learn_hidden=False,
                                    doclbl_vectors=doclbl_vectors, doclbl_locks=doclbl_locks)
            elif self.dm_concat:
                train_sentence_dm_concat(self, word_vocabs, doclbl_indices, alpha, work, neu1,
                                         learn_words=False, learn_hidden=False,
                                         doclbl_vectors=doclbl_vectors, doclbl_locks=doclbl_locks)
            else:
                train_sentence_dm(self, word_vocabs, doclbl_indices, alpha, work, neu1,
                                  learn_words=False, learn_hidden=False,
                                  doclbl_vectors=doclbl_vectors, doclbl_locks=doclbl_locks)
