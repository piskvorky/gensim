...
try:
    ...
    def train_batch_sg(model, sentences, alpha, work=None):
        ...
        for sentence in sentences:
            ...
            for pos, word in enumerate(word_vocabs):
                ...
                # now go over all words from the (reduced) window, predicting
                # each one in turn
        ...
        for sentence in sentences:
            ...
            for pos, word in enumerate(word_vocabs):
                ...
                window_pos = enumerate(
                    ...
                    start)
                word2_indices = [
                    word2.index for pos2, word2 in window_pos if (
                        word2 is not None and pos2 != pos)]
                l1 = np_sum(
                    model.syn0[word2_indices],
    ...
    def score_sentence_sg(model, sentence, work=None):
        ...
        for pos, word in enumerate(word_vocabs):
            ...
            # now go over all words from the window, predicting each one in
            # turn
    ...
    def score_sentence_cbow(model, sentence, alpha, work=None, neu1=None):
        ...
        for pos, word in enumerate(word_vocabs):
            ...
            window_pos = enumerate(
                ...
                start)
            word2_indices = [
                word2.index for pos2,
                word2 in window_pos if (
                    word2 is not None and pos2 != pos)]
            l1 = np_sum(model.syn0[word2_indices], axis=0)  # 1 x layer1_size
            if word2_indices and model.cbow_mean:
                l1 /= len(word2_indices)
            log_prob_sentence += score_cbow_pair(model,
                                                 word, word2_indices, l1)

        return log_prob_sentence

# If pyemd C extension is available, import it.
# If pyemd is attempted to be used, but isn't installed, ImportError will
# be raised.
try:
...
def train_sg_pair(
    ...
    if learn_vectors:
        # learn input -> hidden (mutates model.syn0[word2.index], if that is
        # l1)
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
        ...
        if FAST_VERSION == -1:
            logger.warning(
                'Slow version of {0} is being used'.format(__name__))
        else:
    ...
    def create_binary_tree(self):
        ...
        """
        logger.info(
            "constructing a huffman tree from %i words", len(
                self.vocab))
        ...
        # recurse over the tree, assigning a binary code to each vocabulary
        # word
        if heap:
            ...
            while stack:
                ...
                if node.index < len(self.vocab):
                    ...
                    points = array(
                        list(points) + [node.index - len(self.vocab)], dtype=uint32)
                    stack.append(
                        (node.left,
                         array(
                             list(codes) + [0],
            ...
            logger.info(
                "built huffman tree with maximum node depth %i",
                max_depth)
    ...
    def scan_vocab(
        ...
        for sentence_no, sentence in enumerate(sentences):
            ...
            if sentence_no % progress_per == 0:
                logger.info(
                    "PROGRESS: at sentence #%i, processed %i words, keeping %i word types", sentence_no, sum(
                        itervalues(vocab)) + total_words, len(vocab))
    ...
    def scale_vocab(
                ...
                if keep_vocab_item(word, v, min_count, trim_rule=trim_rule):
                    ...
                    if not dry_run:
                        self.vocab[word] = Vocab(
                            count=v, index=len(self.index2word))
        ...
        if not sample:
            ...
            # new shorthand: sample >= 1 means downsample all words with higher
            # count than sample
        ...
        if not dry_run and not keep_raw_vocab:
            logger.info(
                "deleting the raw counts dictionary of %i items", len(
                    self.raw_vocab))
            self.raw_vocab = defaultdict(int)

        logger.info(
            "sample=%g downsamples %i most-common words",
            sample,
            downsample_unique)
    ...
    def sort_vocab(self):
        ...
        self.index2word.sort(key=lambda word: self.vocab[
                             word].count, reverse=True)
    ...
    def train(self, sentences, total_words=None, word_count=0,
        ...
        if FAST_VERSION < 0:
            ...
            self.neg_labels = []
            if self.negative > 0:
                # precompute negative labels optimization for pure-python
                # training
        ...
        def job_producer():
            ...
            for sent_idx, sentence in enumerate(sentences):
                ...
                if batch_size + sentence_length <= self.batch_words:
                    ...
                    # add the sentence that didn't fit as the first item of a
                    # new job
                    job_batch, batch_size = [sentence], sentence_length

            # add the last job too (may be significantly smaller than
            # batch_words)
            if job_batch:
                logger.debug(
                    "queueing job #%i (%i words, %i sentences) at alpha %.05f",
        ...
        # buffer ahead only a limited number of jobs.. this is the reason we
        # can't simply use ThreadPool :(
    ...
    def score(
        ...
        logger.info(
            ...
            "using sg=%s hs=%s sample=%s and negative=%s", self.workers, len(
                self.vocab), self.layer1_size, self.sg, self.hs, self.sample, self.negative)
        ...
        while True:
            try:
                ...
                logger.info(
                    ...
                    job_no - done_jobs + 1)
                for _ in xrange(self.workers):
                    # give the workers heads up that they can finish -- no more
                    # work!
                    job_queue.put(None)
                push_done = True
            try:
    ...
    def update_weights(self):
        ...
        for i in xrange(len(self.syn0), len(self.vocab)):
            ...
            newsyn0[
                i - len(self.syn0)] = self.seeded_vector(self.index2word[i] + str(self.seed))
        ...
        self.syn0_lockf = ones(len(self.vocab),
    ...
    def reset_weights(self):
        ...
        for i in xrange(len(self.vocab)):
            # construct deterministic seed from word AND seed argument
            self.syn0[i] = self.seeded_vector(
                self.index2word[i] + str(self.seed))
        if self.hs:
    ...
    def seeded_vector(self, seed_string):
        ...
        # Note: built-in hash() may vary by Python version or even (in Py3.x)
        # per launch
    ...
    def save_word2vec_format(self, fname, fvocab=None, binary=False):
        ...
        if fvocab is not None:
            ...
            with utils.smart_open(fvocab, 'wb') as vout:
                for word, vocab in sorted(iteritems(
                        self.vocab), key=lambda item: -item[1].count):
        ...
        with utils.smart_open(fname) as fin:
            ...
            def add_word(word, weights):
                ...
                if word in result.vocab:
                    logger.warning(
                        "duplicate word '%s' in %s, ignoring all but first", word, fname)
                    return
                if counts is None:
    ...
    def intersect_word2vec_format(
        ...
        with utils.smart_open(fname) as fin:
            ...
            if not vector_size == self.vector_size:
                raise ValueError(
                    "incompatible vector size %d in file %s" %
                    (vector_size, fname))
                # TOCONSIDER: maybe mismatched vectors still useful enough to
                # merge (truncating/padding)?
            if binary:
                binary_len = dtype(REAL).itemsize * vector_size
                for line_no in xrange(vocab_size):
                    # mixed text and binary: read text first, then binary
                    ...
                    if word in self.vocab:
                        ...
                        self.syn0_lockf[self.vocab[word].index] = lockf
            else:
        ...
        logger.info("merged %d vectors into %s matrix from %s" %
                    (overlap_count, self.syn0.shape, fname))
    ...
    def most_similar(
        ...
        if isinstance(positive, string_types) and not negative:
            # allow calls like most_similar('dog'), as a shorthand for
            # most_similar(['dog'])
            positive = [positive]
    ...
    def wmdistance(self, document1, document2):
        ...
        if not PYEMD_EXT:
            raise ImportError(
                "Please install pyemd Python package to compute WMD.")
    ...
    def most_similar_cosmul(self, positive=[], negative=[], topn=10):
        ...
        if isinstance(positive, string_types) and not negative:
            # allow calls like most_similar_cosmul('dog'), as a shorthand for
            # most_similar_cosmul(['dog'])
            positive = [positive]
    ...
    def doesnt_match(self, words):
        ...
        vectors = vstack(self.syn0norm[self.vocab[word].index]
                         for word in words).astype(REAL)
    ...
    def __getitem__(self, words):
        ...
        if isinstance(words, string_types):
            # allow calls like trained_model['office'], as a shorthand for
            # trained_model[['office']]
    ...
    def init_sims(self, replace=False):
        ...
        if getattr(self, 'syn0norm', None) is None or replace:
            ...
            if replace:
                ...
                self.syn0norm = (
                    self.syn0 / sqrt((self.syn0 ** 2).sum(-1))[..., newaxis]).astype(REAL)
    ...
    def accuracy(
        ...
        for line_no, line in enumerate(utils.smart_open(questions)):
            # TODO: use level3 BLAS (=evaluate multiple questions at once), for
            # speed
            ...
            if line.startswith(': '):
                ...
                try:
                    if case_insensitive:
                        a, b, c, expected = [word.upper()
                                             for word in line.split()]
                    else:
                        a, b, c, expected = [word for word in line.split()]
                except:
                    logger.info(
                        "skipping invalid line #%i in %s" %
                        (line_no, questions))
                    continue
                ...
                predicted = None
                # find the most likely prediction, ignoring OOV words and input
                # words
                sims = most_similar(
                    self,
                    positive=[
    ...
    def save(self, *args, **kwargs):
        # don't bother storing the cached normalized vectors, recalculable
        # table
    ...
    def __iter__(self):
        for fname in os.listdir(self.dirname):
            ...
            for line in utils.smart_open(fname):
                ...
                token_tags = [t.split('/')
                              for t in line.split() if len(t.split('/')) == 2]
                # ignore words with non-alphabetic tags like ",", "!" etc
                # (punctuation, weird stuff)
                words = [
                    "%s/%s" %
                    (token.lower(), tag[
                        :2]) for token, tag in token_tags if tag[
                        :2].isalpha()]
...
class Text8Corpus(object):
    ...
    def __iter__(self):
        ...
        # so just split the sequence of tokens arbitrarily: 1 sentence = 1000
        # tokens
        sentence, rest = [], b''
        with utils.smart_open(self.fname) as fin:
            while True:
                ...
                if text == rest:  # EOF
                    ...
                    # return the last chunk of words, too (may be
                    # shorter/longer)
                    sentence.extend(words)
                    if sentence:
                        yield sentence
                    break
                # last token may have been split in two... keep for next
                # iteration
                last_token = text.rfind(b' ')
                words, rest = (
                    utils.to_unicode(
                        text[
                            :last_token]).split(), text[
...
# Example: ./word2vec.py -train data.txt -output vec.txt -size 200 -window
# 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -cbow 1 -iter 3
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-threads",
        help="Use THREADS threads (default 12)",
        type=int,
        default=12)
    parser.add_argument(
        "-iter",
