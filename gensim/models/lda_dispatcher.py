...
class Dispatcher(object):
    ...
    def __init__(self, maxsize=MAX_JOBS_QUEUE, ns_conf={}):
        ...
        # a pyro proxy to this object (unknown at init time, but will be set
        # later)
        self.callback = None
    ...
    def initialize(self, **model_params):
        ...
        # locate all available workers and store their proxies, for subsequent
        # RMI calls
        self.workers = {}
        with utils.getNS(**self.ns_conf) as ns:
            self.callback = Pyro4.Proxy(ns.list(prefix=LDA_DISPATCHER_PREFIX)[
    ...
    def putjob(self, job):
        ...
        logger.info(
            "added a new job (len(queue)=%i items)" %
            self.jobs.qsize())
...
def main():
    ...
    parser.add_argument(
        "--maxsize",
        help="How many jobs (=chunks of N documents) "
