...
class TestWord2VecModel(unittest.TestCase):
    ...
    def testPersistenceWord2VecFormat(self):
        ...
        binary_model = word2vec.Word2Vec.load_word2vec_format(
            testfile(), binary=True)
        binary_model.init_sims(replace=False)
        self.assertTrue(numpy.allclose(model['human'], binary_model['human']))
        norm_only_model = word2vec.Word2Vec.load_word2vec_format(
            testfile(), binary=True)
        ...
        self.assertTrue(
            numpy.allclose(
                model.syn0norm[
                    model.vocab['human'].index],
                norm_only_model['human']))
        limited_model = word2vec.Word2Vec.load_word2vec_format(
            testfile(), binary=True, limit=3)
        self.assertEquals(len(limited_model.syn0), 3)
        half_precision_model = word2vec.Word2Vec.load_word2vec_format(
            testfile(), binary=True, datatype=numpy.float16)
        self.assertEquals(
            binary_model.syn0.nbytes,
            half_precision_model.syn0.nbytes * 2)
    ...
    def testPersistenceWord2VecFormatNonBinary(self):
        ...
        text_model = word2vec.Word2Vec.load_word2vec_format(
            testfile(), binary=False)
    ...
    def testPersistenceWord2VecFormatWithVocab(self):
        ...
        model = word2vec.Word2Vec(sentences, min_count=1)
        model.init_sims()
        testvocab = os.path.join(
            tempfile.gettempdir(),
            'gensim_word2vec.vocab')
    ...
    def testVocab(self):
        ...
        # with min_count=1, we're not throwing away anything, so make sure the
        # word counts add up to be the entire corpus
        self.assertEqual(
            sum(v.count for v in model.vocab.values()), total_words)
    ...
    def testTraining(self):
        ...
        sims2 = [(w, sim) for w, sim in sims2 if w !=
                 'graph']  # ignore 'graph' itself
        self.assertEqual(sims, sims2)

        # build vocab and train in one step; must be the same as above
        model2 = word2vec.Word2Vec(
            sentences, size=2, min_count=1, hs=1, negative=0)
    ...
    def testLocking(self):
        ...
        for sg in range(2):  # test both cbow and sg
            model = word2vec.Word2Vec(
                size=4, hs=1, negative=5, min_count=1, sg=sg, window=5)
            model.build_vocab(corpus)
            ...
            self.assertFalse((unlocked1 == model.syn0[1]).all())
            # locked vector should not vary
            self.assertTrue((locked0 == model.syn0[0]).all())
    ...
    def testTrainingSgNegative(self):
        ...
        # build vocab and train in one step; must be the same as above
        model2 = word2vec.Word2Vec(
            sentences, size=2, min_count=1, hs=0, negative=2)
        self.models_equal(model, model2)
    ...
    def testSimilarities(self):
        ...
        self.assertTrue(model.n_similarity(
            ['graph', 'trees'], ['trees', 'graph']))
        self.assertTrue(
            model.n_similarity(
                ['graph'],
                ['trees']) == model.similarity(
                'graph',
                'trees'))
        self.assertRaises(
            ZeroDivisionError, model.n_similarity, [
                'graph', 'trees'], [])
        self.assertRaises(
            ZeroDivisionError, model.n_similarity, [], [
                'graph', 'trees'])
    ...
    def models_equal(self, model, model2):
        ...
        most_common_word = max(
            model.vocab.items(),
            key=lambda item: item[1].count)[0]
        self.assertTrue(
            numpy.allclose(
                model[most_common_word],
