...
class TestWord2VecModel(unittest.TestCase):
    ...
    def testMultistreamTraining(self):
        ...
        # build vocabulary, don't train yet
        input_streams = [sentences[:len(sentences) / 2], sentences[len(sentences) / 2:]]
        model = word2vec.Word2Vec(size=2, min_count=1, hs=1, negative=0, workers=3)
        ...
        # build vocab and train in one step; must be the same as above
        model2 = word2vec.Word2Vec(input_streams, size=2, min_count=1, hs=1, negative=0, multistream=True, workers=3)
        self.models_equal(model, model2)

        # train singlestream model; must be the same as above
        model3 = word2vec.Word2Vec(sentences, size=2, min_count=1, hs=1, negative=0, workers=3)
