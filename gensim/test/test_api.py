...
class TestApi(unittest.TestCase):
    ...
    def test_load_dataset(self):
        dataset_path = os.path.join(
            api.BASE_DIR, "__testing_matrix-synopsis", "__testing_matrix-synopsis.gz"
        )
        if os.path.isdir(api.BASE_DIR):
            shutil.rmtree(api.BASE_DIR)
        self.assertEqual(
            api.load("__testing_matrix-synopsis", return_path=True), dataset_path
        )
        shutil.rmtree(api.BASE_DIR)
    ...
    def test_load_model(self):
        ...
        dataset_path = os.path.join(
            ...
            "__testing_word2vec-matrix-synopsis.gz",
        )
        ...
        shutil.rmtree(api.BASE_DIR)
        self.assertEqual(
            api.load("__testing_word2vec-matrix-synopsis", return_path=True),
            dataset_path,
        )
        shutil.rmtree(api.BASE_DIR)
    ...
    def test_multipart_load(self):
        dataset_path = os.path.join(
            ...
            "__testing_multipart-matrix-synopsis.gz",
        )
        if os.path.isdir(api.BASE_DIR):
            shutil.rmtree(api.BASE_DIR)
        self.assertEqual(
            dataset_path,
            api.load("__testing_multipart-matrix-synopsis", return_path=True),
        )
        shutil.rmtree(api.BASE_DIR)
