...
class TestLdaModel(unittest.TestCase, basetmtests.TestBaseTopicModel):
    def setUp(self):
        ...
        # test non-standard port to see if it still locates the right nameserver
        self.host = '127.0.0.1'
        self.port = 3123
        env = {
        ...
        self.ns_process = Subprocess.Popen([sys.executable, '-m', 'Pyro4.naming', '--host=' + host, '--port=' + str(port)], env=env)
        self.worker_processes = [
            Subprocess.Popen([sys.executable, '-m', 'gensim.models.lsi_worker', '--host', host, '--port', str(port)], env=env)
            for _ in range(2)
        ]
        self.dispatcher_process = Subprocess.Popen([sys.executable, '-m', 'gensim.models.lsi_dispatcher', '--host', host, '--port', str(port)], env=env)
        self.model = self.class_(corpus, id2word=dictionary, num_topics=2, passes=100, distributed=True, ns_conf=dict(host=self.host, port=self.port))
