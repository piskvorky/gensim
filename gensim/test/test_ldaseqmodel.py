...
class TestSSLM(unittest.TestCase):
    def testUpdateZeta(self):
        ...
        sslm = ldaseqmodel.sslm(mean=mean, variance=variance, obs=obs, zeta=zeta, num_terms=562, num_sequences=4)
    ...
    def testPostMean(self):
        ...
        sslm = ldaseqmodel.sslm(chain_variance=chain_variance, obs_variance=0.5, num_terms=562, num_sequences=4, variance=variance, fwd_variance=fwd_variance, mean=mean, fwd_mean=fwd_mean, obs=obs)
    ...
    def testLogProb(self):
        ...
        sslm = ldaseqmodel.sslm(chain_variance=chain_variance, obs_variance=0.5, num_terms=562, num_sequences=4, mean=mean, zeta=zeta, e_log_prob=e_log_prob)
    ...
    def testMeanDeriv(self):
        ...
        sslm = ldaseqmodel.sslm(num_sequences=4, variance=variance, obs_variance=0.500000 , chain_variance=0.005000)
    ...
    def testObsDeriv(self): 
        ...
        sslm = ldaseqmodel.sslm(num_sequences=num_sequences, mean=mean, variance=variance, m_update_coeff=m_update_coeff, w_phi_l=w_phi_l, chain_variance=chain_variance, zeta=zeta)
    ...
    def testUpdateBound(self):
        ...
        sslm = ldaseqmodel.sslm(num_terms=num_terms, num_sequences=num_sequences, variance=variance, chain_variance=chain_variance, obs_variance=obs_variance,zeta=zeta, w_phi_l=w_phi_l, mean=mean, fwd_variance=fwd_variance, fwd_mean=fwd_mean, obs=obs)
    ...
    def testDfObs(self):
        ...
        sslm = ldaseqmodel.sslm(obs=obs, num_sequences=4, chain_variance=chain_variance, zeta=zeta, mean=mean, variance=variance, w_phi_l=w_phi_l, m_update_coeff=m_update_coeff, fwd_mean=fwd_mean, fwd_variance=fwd_variance)
        args = sslm, word_counts, totals, mean_deriv_mtx, word

        # val = ldaseqmodel.f_obs(x, args)
        # expected_val = 0.188
