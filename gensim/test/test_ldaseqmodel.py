...
class TestLdaSeq(unittest.TestCase):
        # we are setting up a DTM model and fitting it, and checking topic-word
        # and doc-topic results.
    ...
    def setUp(self):
        texts = [[u'senior',
                 ...
                 ['bank',
                  'loan',
                  'sell']]
        # initializing using own LDA sufficient statistics so that we get same
        # results each time.
        sstats = numpy.loadtxt(datapath('sstats_test.txt'))
        dictionary = Dictionary(texts)
        corpus = [dictionary.doc2bow(text) for text in texts]
        self.ldaseq = ldaseqmodel.LdaSeqModel(
            corpus=corpus, id2word=dictionary, num_topics=2, time_slice=[
                10, 10, 11], initialize='own', sstats=sstats)
