...
try:
    ...
    # no SVDLIBC: install with `easy_install sparsesvd` if you want SVDLIBC
    # results as well
    sparsesvd = None
...
# when reporting reconstruction error, also report spectral norm error?
# (very slow)
COMPUTE_NORM2 = False
...
def norm2(a):
    ...
    if COMPUTE_NORM2:
        ...
        # much faster than numpy.linalg.norm(2)
...
if __name__ == '__main__':
    ...
    for factors in FACTORS:
        ...
        print(
            '*' * 40, "%i factors, ideal error norm_frobenius=%f, norm_2=%f" %
            (factors, ideal_fro, ideal_n2))
        print("*" * 30, end="")
        print_error(
            "baseline", aat, numpy.zeros(
                (m, factors)), numpy.zeros(
                (factors)), ideal_fro, ideal_n2)
        if sparsesvd:
            ...
            del vt
            u, s = ut.T.astype(
                numpy.float32), s.astype(
                numpy.float32)**2  # convert singular values to eigenvalues
            del ut
        ...
        for power_iters in POWER_ITERS:
            for chunksize in CHUNKSIZE:
                ...
                taken = time.time() - taken
                u, s = model.projection.u.astype(
                    numpy.float32), model.projection.s.astype(
                    numpy.float32)**2
                del model
