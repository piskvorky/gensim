...
class TestLsiModel(unittest.TestCase, basetmtests.TestBaseTopicModel):
    ...
    def test_transform(self):
        ...
        u, s, vt = scipy.linalg.svd(
            matutils.corpus2dense(self.corpus, self.corpus.num_terms),
            full_matrices=False,
        )
        self.assertTrue(
            np.allclose(s[:2], model.projection.s)
        ...
        self.assertTrue(
            np.allclose(abs(vec), abs(expected))
    ...
    def test_transform_float32(self):
        ...
        expected = np.array([-0.6594664, 0.142115444])  # scaled LSI version
        # transformed entries must be equal up to sign
        self.assertTrue(np.allclose(abs(vec), abs(expected), atol=1.0e-5))
    ...
    def test_online_transform(self):
        ...
        transformed = model[doc]
        vec = matutils.sparse2full(
            transformed, model.num_topics
        )  # convert to dense vector, for easier equality tests
        expected = np.array([-1.73205078, 0.0, 0.0, 0.0, 0.0])  # scaled LSI version
        self.assertTrue(
            np.allclose(abs(vec), abs(expected), atol=1e-6)
        )  # transformed entries must be equal up to sign

        # train on another 4 documents
        model.add_documents(
            corpus[1:5], chunksize=2
        ...
        expected = np.array(
            [-0.66493785, -0.28314203, -1.56376302, 0.05488682, 0.17123269]
        )  # scaled LSI version
        self.assertTrue(
            np.allclose(abs(vec), abs(expected), atol=1e-6)
        )  # transformed entries must be equal up to sign
