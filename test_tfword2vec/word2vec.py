...
flags.DEFINE_string("ps_hosts", None, "Parameter server hosts")
flags.DEFINE_string("worker_hosts", None, "Worker hosts")
...
class Word2Vec(object):
    ...
    def forward(self, examples, labels):
        ...
        sm_w_t = tf.Variable(tf.zeros([opts.vocab_size, opts.emb_dim]),
                             name="sm_w_t")
...
def main(_):
    ...
    sv = tf.train.Supervisor(is_chief=(opts.task_index == 0), global_step=model.global_step,
                        init_op=model.init)

    with sv.prepare_or_wait_for_session(opts.server.target, config=None) as sess:
        model._session = sess
