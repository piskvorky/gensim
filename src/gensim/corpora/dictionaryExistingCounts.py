...
"""
Mod to dictionary.py.
Instead of being focused on loading docs and generating the counts (addDocuments)
This module assumes that the counts already exists (e.g. read from a file created
using loadDictionary(fname)) and are passed as parameters

Dictionaries can later be pruned according to document frequency
(removing (un)common words via the :func:`Dictionary.filterExtremes` method),
...
class DictionaryExistingCounts(utils.SaveLoad):
    """
    ...
    def __init__(self, word2id, id2word):
        self.token2id = word2id # token -> tokenId
        self.id2token = id2word # token -> tokenId
    ...
    def doc2bow(self, document, allowUpdate = False, returnMissingWords = True):
        ...
        **tokenized and normalized** utf-8 encoded string.
        ...
        if returnMissingWords:
            logger.debug("doc2bow: for query " + str(document)+ " the missing words are:")
