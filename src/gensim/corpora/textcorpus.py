...
The original idea of Gensim was to delegate parsing etc to external tools.
For our usage scenario we do need to have a representation fo a text corpus.
We need basic agreement on what memory structures define a vorpus.
To do functional testing, tests will use a corpus; if we serialize the memmory
s
We assume the input text is one single file with documents separated by blanks.

A text corpus is defined by a parsing and several output files (serializations).
The serializations are both in cpickle format (faster to load into memory) and in text format \
for easier human supervision.

This class has methods to read and write all those serializations.

It creates three files:

* `OUTPUT_PREFIX_wordids.txt`: mapping between words and their integer ids
* `OUTPUT_PREFIX_idwords.txt`: mapping between words and their integer ids
...
1. corpusFilename: a single file with one doc per line. example: wiki2008

2. filters (if none, using DEFAULT_FILTERS). This is a file too: corpusFilename.filters

3. the txt serializations

4. the cpickle serializations

5. a flag to optionally create txt files. by default it doesn't.

To instantiate from txt or cpickle, we assume a convention for representations:
...
The name is: corpusFilename + parsingParameters + representation + '.txt' OR 'cpickle'

We can have either txt OR cpickle or both. txt is only for human debugging. If both are present,\
 load should use the cpickle

1.1 methods

for every representation:

1. loadtxt, loadCpickle

2. save, saveCpickle

3. getArticles. Yields an article (for the model object)
