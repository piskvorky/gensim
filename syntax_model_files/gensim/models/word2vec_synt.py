...
try:
    ...
    def train_batch_cbow(model, sentences, alpha, work=None, neu1=None):
        ...
        for sentence in sentences:
            ...
            for pos, word in enumerate(word_vocabs):
                reduced_window = model.random.randint(model.window)  # `b` in the original word2vec code
                start = max(0, pos - model.window + reduced_window)  # choose starting word position
                window_pos = enumerate(word_vocabs[start:(pos + model.window + 1 - reduced_window)], start) # create a window
...
class Word2Vec(utils.SaveLoad):
    """
    Class for training, using and evaluating neural networks described in https://code.google.com/p/
    /
